...
impl ExecutionPlan for QueryStageExec {
    ...
    async fn execute(
        ...
        Err(DataFusionError::NotImplemented("QueryStageExec does not support a complete execution. ".to_string()
         + "It is designed for distributed query executions and therefore should only run a single input partition. "
         + "Please use execute_input_partition instead."))
    }
    ...
    async fn execute_input_partition(
        ...
        let mut stream = self.plan.execute_input_partition(input_partition, output_partition).await?;
        ...
        info!(
            "Executed output_partition {} in {} seconds. Statistics: {:?}",
            output_partition,
            now.elapsed().as_secs(),
            stats
        );
